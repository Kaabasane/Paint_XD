...
class Paint:
    ...
    ### Tabs
    def create_new_tab(self, tabconfig=None, image=None):
        global gl_cur_tab
        if tabconfig:
        ...
        if tabconfig:
            ...
            img = ImageTk.PhotoImage(Image.open(CONFIG / f"{new_tab}.eps"))
            self.canv.config(width=img.width(), height=img.height())
            self.canv.create_image(0, 0, anchor="nw", image=img)

        elif image:
            
            self.canv.config(width=image.width(), height=image.height())
            self.canv.create_image(0, 0, anchor="nw", image=image)

        self.canv.bind("<ButtonRelease-1>", self.reset)
        self.canv.bind("<Button-1>", self.point)
    ...
    def change_default_colors(self):
        ...
        #         self.canv.postscript(file=fileps)
    ...
    def paint(self, event):
        ...
        if self.old_x and self.old_y:
            ...
            self.canv.create_line(event.x, event.y, event.x, event.y,
                               width=line_width, fill=paint_color,
                               capstyle="round")
        self.old_x = event.x
        self.old_y = event.y
    ...
    def line(self, x, y, line_width):
        ...
        self.canv.create_line(self.old_x, self.old_y, x, y,
                           width=line_width, fill=paint_color,
    ...
    def fill(self, clear=False):
        ...
        color = askcolor(color=self.tab.color)[1]
        if color is not None:
            self.canv.configure(bg=color)

    ### Save
    def save_file(self, event=None):
        path = self.tab.path
        if path is not None:
            fileps = Path(f"{path}.eps")
            self.canv.postscript(file=fileps)
    ...
    def save_as_file(self, event=None):
        file = asksaveasfile(initialfile=self.tab.name, filetypes=FILE_TYPES,
                             defaultextension=FILE_TYPES[0][1][1:])
        if file:
            fileps = Path(f"{file.name}.eps")
            self.canv.postscript(file=fileps)
    ...
    def use_cursor_icons(self):
        if self.use_icons.get():
            if self.tab.active_button["text"] == "Pen":
                ...
                self.canv.config(cursor="dot")
            elif self.tab.active_button["text"] == "Eraser":
                self.canv.config(cursor="dot white black")
            else:
    ...
    def cleanup(self):
        self.root.quit()
        self.save_config()
        for filename in self.hidden_tabs:
            Path(f"{filename}.eps").unlink(missing_ok=True)
        self.root.destroy()
